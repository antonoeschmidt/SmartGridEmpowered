...
contract MarketRevised {
    address owner;
    ICableCompany2 cableCompany; // Maybe change the name of CableCompany to something else, like DSO
    uint maxOfferLivespan = 1000 * 60 * 60 * 24 * 7;
    ISmartMeterRevised smartMeter;
    ...
    constructor(
        ...
        owner = msg.sender;
        cableCompany = ICableCompany2(_cableCompanyAddress);
        cableCompanyAddress = _cableCompanyAddress;
        smartMeter = ISmartMeterRevised(smartMeterContract);
    }
    ...
    /**
     *
     * @param acceptedIndicies All the indicies where the offer was ok.
     * @param maxIndex The maximum index evaluated by the DSO
     */
    function validatePendingOffers(
        uint[] memory acceptedIndicies,
        uint maxIndex
    ) public {
        require(msg.sender == owner, "sender is not owner");
        // emit approved offers
        for (uint i = 0; i < acceptedIndicies.length; i++) {
        ...
        // move not yet validate confirmation offers down.
        uint lastIndex = 0;
        for (uint i = 0; i < maxIndex; i++) {
            ...
            if (acceptedIndicies[lastIndex] == i) {
                emit ApproveOffer(
                    offer.buyerSignature,
                    offer.sellerSignature,
                    offer.amount,
                ...
                // if not hte owner gets their battery back.
            ...
            if (maxIndex < pendingConfirmationOffers.length - 1 + i) {
